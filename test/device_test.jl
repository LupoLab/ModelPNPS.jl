...
@testset "Device support" begin
    ...
    @testset "quadrant_ranges" begin
        ...
        xygrid = Grid.FreeGrid(400.0e-6, 16)
        ...
        bad = copy(sig_quad)
        bad[1, 1] = true
        @test_throws ArgumentError TS.quadrant_ranges(bad)
        @test_throws ArgumentError TS.quadrant_ranges(trues(5, 6))  # odd size
    end
    ...
    if have_jlarrays
        ...
        AbstractFFTs.plan_inv(p::JLPlan) =
            ...
            JLPlan(inv(p.hp).p, p.sz, p.dims, ComplexF64),
        ...
        @testset "build_setup on device" begin
            ...
            @test isapprox(
                Array(TS.delayed_input(sd, 0.7e-15)),
                TS.delayed_input(sh, 0.7e-15); rtol = 1.0e-12
            )
            @test isapprox(
                Array(TS.delayed_input(sb, 0.7e-15)),
                TS.delayed_input(sh, 0.7e-15); rtol = 1.0e-12
            )
        end
        ...
        @testset "SignalQuadrantNorm on device" begin
            ...
            y, yn = mk(), mk()
            ks = ntuple(_ -> mk(), 7)
            host = FakeStepper(y, yn, ks, nothing, 1.3e-7, 1.0e-8, 1.0e-10, qn)
            dev = FakeStepper(
                JLArray(y), JLArray(yn), map(JLArray, ks), nothing,
            ...
            @test isapprox(eh, ed; rtol = 1.0e-11)
            ...
            yerr = @. host.dt * (
                ks[1] * RK45.errest[1] + ks[3] * RK45.errest[3] +
                    ks[4] * RK45.errest[4] + ks[5] * RK45.errest[5] +
                    ks[6] * RK45.errest[6] + ks[7] * RK45.errest[7]
            )
            @test isapprox(eh, qn(yerr, y, yn, host.rtol, host.atol); rtol = 1.0e-11)
        ...
        @testset "save-time extraction" begin
            ...
            sh = TS.build_setup(; base_kwargs...)
            sd = TS.build_setup(; base_kwargs..., arraytype = JLArray)
            zv = [0.0, 0.5e-6, 1.0e-6]
            kwp = (; zsave = zv, init_dz = 5.0e-7, rtol = 1.0e-8)
            ...
            @test isapprox(d.Iω_win, a.Iω_win; rtol = 1.0e-10)
            @test isapprox(d.Iω_win_reimaged, a.Iω_win_reimaged; rtol = 1.0e-10)
            @test isapprox(d.Iω_full, a.Iω_full; rtol = 1.0e-10)
            ...
            @test isapprox(d0.Iω_win, d.Iω_win; rtol = 1.0e-10)
        ...
        @testset "simulate_delay_point on device" begin
            ...
            ohr = TS.simulate_delay_point(shr, 0.0; nz = 2, init_dz = 5.0e-7, rtol = 1.0e-8)
            odr = TS.simulate_delay_point(sdr, 0.0; nz = 2, init_dz = 5.0e-7, rtol = 1.0e-8)
            @test isapprox(odr.Iω_win, ohr.Iω_win; rtol = 1.0e-8)

            # the streamed (HDF5) path works too — that is what run_scan uses
            mktempdir() do dir
                fn = joinpath(dir, "dev.h5")
                outs = TS.simulate_delay_point(
                    sd, 0.0; nz = 2, init_dz = 5.0e-7, rtol = 1.0e-8,
                    filename = fn
                )
                @test isapprox(outs.Iω_win, outh.Iω_win; rtol = 1.0e-8)
            end
        end
