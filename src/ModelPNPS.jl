...
`state`, `et_win`, `eto`, `ewo`, `pto`, `analytic`, `window`, and the totals `device` and
`host`. All in GiB.
...
function memory_budget(setup_args::NamedTuple)
    ...
    _budget(grid, N, field, response, nwin)
end

function _budget(grid, N, field, response, nwin)
    ...
    (; Nω, Nt, Nto, Nωo, field=b(Nω, 16),
       state, et_win, eto, ewo, pto, analytic, window,
       device = state + et_win + eto + ewo + pto + analytic + window,
